...
@Autonomous(name = "FTCAutonomousPPG")
public class FTCAutonomousPPG extends OpMode {
    ...
    // Spit rotation (degrees) before each ball push: [pile][ball]
    // Target: P, P, G — pile arrangements are always upper=PPG, middle=PGP, lower=GPP
    private static final double[][] SPIT_ROT = {
        {0, 240, 240},    // Pre-loaded (standard, tested)
        {0, 120, 120},    // Pile 1 - upper PPG: P(slot0), P(slot1), G(slot2) - natural
        {0, 240, 240},    // Pile 2 - middle PGP: P(slot0), P(slot2), G(slot1)
        {120, 120, 120},  // Pile 3 - lower GPP: P(slot1), P(slot2), G(slot0)
    };
    ...
    public void init() {
        ...
        telemetryM.debug("PPG Autonomous initialized.");
